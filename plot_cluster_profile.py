...
idx_x = lon_rng[0]
idx_y = lat_rng[1]
prof_pnt  = np.array([[103.72,33.325], [103.91,33.05]])
...
plt.figure(figsize=(abs_ab*20,4))
...
for i,cc in enumerate(ccs):       
 ...
 #   plt.scatter(lon, lat, mag, alpha=alpha, color='gray', edgecolor='none', zorder=len(faults)+2)
#    plt.scatter(bg_clust[:,0], bg_clust[:,1], bg_clust[:,2], color='gray', edgecolor='none', linewidths=0.5, alpha=alpha, zorder=len(faults)+2)
   
   # dep = np.array(list(events['dep'])) #add 1008
...
### Mainshock
    ...
    if cos<0 or abs_ac*cos>abs_ab: continue
    prof_dist.append(abs_ac * cos * 111)
    prof_dep.append(12.5)
    prof_mag.append((6.5 + mag_corr*2) * mark_size)
          #  print(prof_dep)
    plt.scatter(prof_dist, prof_dep, prof_mag, alpha=alpha, color='red')


    for j,clust in enumerate(clusts):
       # plt.scatter(clust[:,0], clust[:,1], clust[:,2], alpha=alpha, color=colors[j%num_colors], edgecolor='none', zorder=len(faults)+3)
    #    print(j)
    #    print(clust[:,0])        
